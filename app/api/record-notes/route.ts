import { findRecordTarget, storedNoteScope } from '@/app/features/records/infrastructure/record-target-repository';
import { belongsToTarget, type NoteTarget, type StoredNoteScope } from '@/app/features/records/domain/record-policy';
...
async function contextFor(request: Request, farmId: string, predictionId: string, sessionId: string) {
  ...
  const target = await findRecordTarget(env.DB, farmId, predictionId, sessionId);
...
export async function GET(request: Request) {
  ...
  if (!farmId || (!predictionId && !sessionId)) {
    return NextResponse.json({ error: '메모를 확인할 판독 기록을 선택해 주세요.' }, { status: 400 });
  }
  ...
  if (!context) return NextResponse.json({ error: '이 판독 기록의 메모를 볼 권한이 없습니다.' }, { status: 403 });
...
export async function POST(request: Request) {
  try {
    ...
    if (action === 'TRANSLATE_NOTES') {
      ...
      const notes = (await Promise.all(noteIds.map((noteId) => storedNoteScope(env.DB, 
        noteId,
        farmId,
    ...
    const context = await contextFor(request, farmId, predictionId, sessionId);
    if (!context?.member.id || !context.member.permissions.reviewAlerts) {
      return NextResponse.json({ error: '이 판독 기록에 메모를 남길 권한이 없습니다.' }, { status: 403 });
    }
    const detected = detectSupportedLanguage(content, languageHint, context.member.preferredLanguage);

    let rootParentId: string | null = null;
    if (parentNoteId) {
      const parent = await storedNoteScope(env.DB, parentNoteId, farmId, context.target.capture_session_id);
...
export async function PATCH(request: Request) {
  try {
    ...
    const existing = await storedNoteScope(env.DB, noteId, farmId, context.target.capture_session_id);
