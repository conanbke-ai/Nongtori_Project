...
export function RecordNotes({
  ...
  function evidenceUrl(modality: 'rgb' | 'thermal', targetPredictionId = activePredictionId) {
    ...
    return `/api/record-evidence?${params}`;
  }
  ...
  async function loadNotes(targetPredictionId = activePredictionId) {
    ...
    try {
      ...
      const response = await fetch(`/api/record-notes?${params}`, { cache: 'no-store' });
  ...
  async function requestSharedTranslations(notes: RecordNote[], targetPredictionId: string) {
    ...
    for (const notesBatch of batches) {
      try {
        const response = await fetch('/api/record-notes', {
          method: 'POST',
  ...
  async function remove(note: RecordNote) {
    ...
    try {
      ...
      const response = await fetch(`/api/record-notes?${params}`, { method: 'DELETE' });
      await response.json();
      if (!response.ok) throw new Error(translate(language, 'alert.sharedNoteSaveFailed'));
  ...
  return (
    <section className="record-notes">
      <button aria-expanded={open} className="record-notes-toggle" onClick={toggle} type="button">
        <span>{frameMode ? translate(language, 'alert.frameDiscussions') : recordText(language, 'comments')}</span>
        <b>{(frameMode ? framesLoaded : loaded) ? noteCount : initialCount || ''}</b>
        <i>{translate(language, open ? 'alert.hideSharedNotes' : 'alert.showSharedNotes')}</i>
      </button>
      {open && (
        <div className="record-notes-panel">
          <div className="record-discussion-tools"><p>{recordText(language, 'commentsHelp')}</p><button disabled={loading || sending} type="button" onClick={() => { if (frameMode && !activePredictionId) void loadFrames(); else void loadNotes(activePredictionId); }}>{recordText(language, 'refresh')}</button></div>
