...
export function MiteCaptureForm({ farmId, itemId, language, onUploaded }: { farmId: string; itemId: string; language: Language; onUploaded: () => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch('/api/mite-capture', { method: 'POST', body });
      await response.json();
