...
export type PestAlertPagination = {
  status: 'OPEN' | 'DONE';
  page: number;
  limit: number;
  total: number;
...
function verdictName(value: string | null, language: Language) {
  if (value === 'TARGET_CONFIRMED') return pestText(language, 'confirmed');
  if (value === 'NOT_TARGET') return pestText(language, 'notTarget');
...
export function PestAlertReview({
  alerts,
  farmId,
  canReview,
  language,
  ...
  canReview: boolean;
  language: Language;
  pagination: PestAlertPagination;
  ...
  async function submit(alert: PestAlertRecord, verdict: 'TARGET_CONFIRMED' | 'NOT_TARGET' | 'RECHECK') {
    setSending(alert.id);
    setMessage('');
    setError('');
    try {
