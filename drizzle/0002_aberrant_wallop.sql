CREATE TABLE `cultivars` (
	...
	`supports_quality` integer NOT NULL,
	`supports_ripeness` integer NOT NULL,
	`supports_mite_warning` integer NOT NULL,
	`created_at` text NOT NULL
);
--> statement-breakpoint
CREATE TABLE `fruit_assessments` (
	`id` text PRIMARY KEY NOT NULL,
	`cultivar` text NOT NULL,
	`object_key` text NOT NULL,
	`original_name` text NOT NULL,
	`grade_prediction` text,
	`ripeness_prediction` text,
	`confidence` real,
...
--> statement-breakpoint
CREATE UNIQUE INDEX `fruit_assessments_object_key_unique` ON `fruit_assessments` (`object_key`);--> statement-breakpoint
CREATE INDEX `idx_fruit_assessments_created` ON `fruit_assessments` (`created_at`);--> statement-breakpoint
