...
PATIENCE = 12
MAX_EPOCHS = 25
EXPERIMENT_ID = 'RIPENESS-V008-PATIENCE-DIAGNOSTIC'
...
def run(cache: dict[str, Any], out: Path, logger: RunLogger, seed: int) -> dict[str, Any]:
    ...
    def loader(records, train_mode):
        ...
        return DataLoader(
            DS(records,train_mode), batch_size=32, shuffle=train_mode,
    ...
    def evaluate():
        ...
        with torch.no_grad():
            for x,y in loader(valid_records,False):
                ...
                loss_sum += criterion(z,y).item()*len(y); n+=len(y)
                yt.extend(CLASS_VALUES[i] for i in y.cpu().tolist())
                yp.extend(CLASS_VALUES[i] for i in z.argmax(1).cpu().tolist())
        return loss_sum/n, metrics(yt,yp)

    logger.emit(
        'INFO','TRAINING_STARTED','patience diagnostic training started',
        phase='PATIENCE_DIAGNOSTIC',learning_rate=LR,patience=PATIENCE,
        max_epochs=MAX_EPOCHS,train_samples=len(train_records),
        valid_samples=len(valid_records),device=str(device),
    )

    for epoch in range(1,MAX_EPOCHS+1):
        logger.emit(
            'INFO','EPOCH_STARTED',f'epoch {epoch}/{MAX_EPOCHS} started',
            phase='PATIENCE_DIAGNOSTIC',epoch=epoch,max_epochs=MAX_EPOCHS,
            learning_rate=LR,
        )
        ...
        if improved:
            best=vm['macro_f1']; best_epoch=epoch; best_metrics=vm; wait=0
            torch.save(model.state_dict(),checkpoint)
            logger.emit(
                'INFO','CHECKPOINT_SAVED','patience diagnostic checkpoint improved',
                phase='PATIENCE_DIAGNOSTIC',epoch=epoch,metric_value=best,
        ...
        row={
            ...
            'ordinal_mae':vm['ordinal_mae'],
            'weighted_kappa':vm['weighted_kappa'],
            'm1_f1':vm['per_class']['1']['f1'],
            'best_epoch':best_epoch,
            'no_improve_count':wait,
            'epoch_elapsed_sec':round(time.monotonic()-started,3),
        }
        history.append(row)
        logger.emit(
            'INFO','EPOCH_COMPLETED','patience diagnostic epoch completed',
            phase='PATIENCE_DIAGNOSTIC',learning_rate=LR,
            early_stopping_counter=wait,patience_limit=PATIENCE,
            checkpoint_saved=improved,**row,
        )
        if wait>=PATIENCE:
            logger.emit(
                'INFO','EARLY_STOPPING_TRIGGERED','extended patience exhausted',
                phase='PATIENCE_DIAGNOSTIC',epoch=epoch,best_epoch=best_epoch,
                best_valid_macro_f1=best,patience_limit=PATIENCE,
            )
            break

    result={
        'experiment':EXPERIMENT_ID,'seed':seed,'learning_rate':LR,
        'patience':PATIENCE,'max_epochs':MAX_EPOCHS,'test_evaluated':False,
        'best_epoch':best_epoch,'best_valid_metrics':best_metrics,
        'history':history,'stopped_epoch':history[-1]['epoch'],
    }
...
def main():
    ...
    ap.add_argument('--workdir',type=Path,default=Path('artifacts/ripeness-v008-patience'))
    ...
    a=ap.parse_args()
    logger=RunLogger(a.workdir,'ripeness_v008_patience')
    try:
        logger.emit(
            'INFO','RUN_STARTED','patience diagnostic started',phase='INIT',
            experiment_id=EXPERIMENT_ID,snapshot_id='KGCV-RIPENESS-V001',
            seed=a.seed,test_evaluated=False,cache_root=str(a.cache_root),
        ...
        logger.finish_summary(
            status='SUCCESS',summary_path=a.workdir/'summaries'/'run_summary.json',
            final_metrics={'best_epoch':result['best_epoch'],**result['best_valid_metrics']},
        )
    except Exception as exc:
        logger.exception('RUN_FAILED','patience diagnostic failed',exc,phase='FAILED')
        logger.finish_summary(status='FAILED',summary_path=a.workdir/'summaries'/'run_summary.json')
        raise
