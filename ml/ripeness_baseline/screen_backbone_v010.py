...
def train_convnext_tiny(
    ...
    *,
    ...
    model.classifier[2] = nn.Linear(model.classifier[2].in_features, len(CLASS_VALUES))
    model.to(device)
    ...
    exp = f"CONVNEXT-TINY-LR-{BASE_LR:.0e}-seed-{seed}"
    ...
    logger.emit(
        "INFO", "SCREENING_STARTED", "ConvNeXt-Tiny architecture screening started",
        phase="SCREENING", experiment=exp, change_category="ARCHITECTURE",
        architecture="convnext_tiny", pretrained="IMAGENET1K_V1", seed=seed,
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        if improved:
            ...
            logger.emit(
                "INFO", "CHECKPOINT_SAVED", "ConvNeXt-Tiny validation checkpoint improved",
    ...
    if best_metrics is None:
        raise RuntimeError("no ConvNeXt-Tiny checkpoint created")
...
def main():
    ...
    parser.add_argument("--workdir", type=Path, default=Path("artifacts/ripeness-v010-convnext-tiny-local-gpu"))
    parser.add_argument("--epochs", type=int, default=MAX_EPOCHS)
    parser.add_argument("--seed", type=int, default=SEED)
    args = parser.parse_args()

    logger = RunLogger(args.workdir, "ripeness_v010_convnext_tiny")
    try:
        logger.emit(
            "INFO", "RUN_STARTED", "RIPENESS-V010 ConvNeXt-Tiny screening started",
            phase="INIT", experiment_id="RIPENESS-V010-CONVNEXT-TINY",
        ...
        candidate = train_convnext_tiny(cache, args.workdir, logger, seed=args.seed, max_epochs=args.epochs)
        comparison = {
            "status": "SCREENING_COMPLETE",
            "test_evaluated": False,
            "field_data_used": False,
            "controlled_change": "efficientnet_b0_vs_convnext_tiny",
            "baseline": baseline,
            "candidate": candidate,
        }
        ...
        logger.finish_summary(
            ...
            final_metrics={
                "efficientnet_b0": baseline["best_valid_metrics"],
                "convnext_tiny": candidate["best_valid_metrics"],
            },
        )
    except Exception as exc:
        logger.exception("RUN_FAILED", "V010 ConvNeXt screening failed", exc, phase="FAILED")
